...
"""
@dataclass
class BaseModelOutputWithPastAndCrossAttentionsAndActivations(ModelOutput):
    """
...
class XGLMAttention(nn.Module):
    ...
    def forward(
        ...
        # get query proj
        query_states = self.q_proj(hidden_states,language_ids) * self.scaling
        # get key, value proj
        if is_cross_attention and past_key_value is not None:
            ...
            # cross_attentions
            key_states = self._shape(self.k_proj(key_value_states,language_ids), -1, bsz)
            value_states = self._shape(self.v_proj(key_value_states,language_ids), -1, bsz)
            
        elif past_key_value is not None:
            # reuse k, v, self_attention
            key_states = self._shape(self.k_proj(hidden_states,language_ids), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states,language_ids), -1, bsz)
        ...
        attn_output = self.out_proj(attn_output,language_ids)
...
class XGLMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = activations = self.activation_fn(self.fc1(hidden_states))
...
class XGLMModel(XGLMPreTrainedModel):
    ...
    @add_code_sample_docstrings(
        ...
        output_type=BaseModelOutputWithPastAndCrossAttentionsAndActivations,
    ...
    def forward(
    ...
    ) -> Union[Tuple[torch.Tensor], BaseModelOutputWithPastAndCrossAttentionsAndActivations]:
        r"""
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
        ...
        all_activations = () if output_activations else None
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_cross_attentions, all_activations]
                if v is not None
            )
        return BaseModelOutputWithPastAndCrossAttentionsAndActivations(
...
class XGLMForCausalLM(XGLMPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids.size(0) != language_ids.size(0):
            bi, bl = input_ids.size(0), language_ids.size(0)
            assert bi % bl == 0
            language_ids = language_ids.unsqueeze(1).repeat(1,bi//bl,1).reshape(bi,2)
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, use_cache=None, language_ids=None, **kwargs
    ):
