...
from .tuners import LoraModel, PrefixEncoder, PromptEmbedding, PromptEncoder, LoraXConfig, LoraXModel, PrefixXEncoder
from .utils import (
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig):
        ...
        elif isinstance(self.peft_config, LoraXConfig):
            self.base_model = LoraXModel(peft_config,model)
        else:
    ...
    def _setup_prompt_encoder(self):
        ...
        elif self.peft_config.peft_type == PeftType.PREFIX_TUNING_X:
            prompt_encoder = PrefixXEncoder(self.peft_config)
        else:
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PREFIX_TUNING_X]:
            past_key_values = self.get_prompt(batch_size)
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            if model_kwargs["past_key_values"] is None and self.peft_config.peft_type in [PeftType.PREFIX_TUNING, PeftType.PREFIX_TUNING_X]:
